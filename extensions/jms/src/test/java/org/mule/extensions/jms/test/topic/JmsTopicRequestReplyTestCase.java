...
public class JmsTopicRequestReplyTestCase extends JmsAbstractTestCase {
  ...
  protected String[] getConfigFiles() {
    return new String[] {
        ...
        "operations/jms-topic-request-reply.xml"
    };
  }
  ...
  public void requestReplyExplicitReplyDestination() throws Exception {
    ...
    Future<InternalMessage> requesterTarget = executor.submit(
                                                              () -> flowRunner(REQUEST_LISTENER_FLOW)
                                                                  .withVariable(REQUEST_DESTINATION_VAR, REQUEST_DESTINATION)
                                                                  .run().getMessage());

    Future<InternalMessage> requestReply = executor.submit(
                                                           () -> flowRunner(REQUEST_REPLY_FLOW)
                                                               .withVariable(REQUEST_DESTINATION_VAR, REQUEST_DESTINATION)
    ...
    // Post a message to be read when listening for a reply in the "replyTo" topic
